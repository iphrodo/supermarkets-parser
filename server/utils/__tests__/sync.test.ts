...
describe('runDailySync', () => {
  it('publishes a fresh snapshot when all three sources succeed', async () => {
    ...
    expect(result.snapshot!.offers).toHaveLength(3)
    expect(writeSnapshot).toHaveBeenCalledTimes(1)
  })

  it('carries forward a failed source’s last known-good offers when the other two succeed', async () => {
  ...
  it('publishes with only Billa fresh when Kaufland and Lidl both fail', async () => {
    const previous: DealsSnapshot = {
  ...
  it('leaves the previous snapshot untouched and does not publish when all three sources fail', async () => {
    const writeSnapshot = vi.fn()
