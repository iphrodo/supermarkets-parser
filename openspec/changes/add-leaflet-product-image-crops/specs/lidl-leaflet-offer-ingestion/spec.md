...
For the current weekly leaflet, the system SHALL fetch every page image, in page order, using only publicly accessible leaflet-platform endpoints (no authentication bypass or access-control circumvention). The system SHALL distinguish the image variant submitted for extraction from a smaller variant recorded for later display, and SHALL record each page's display variant together with its pixel dimensions. Where the platform serves only a fixed set of signed image variants, the system SHALL select from those variants and SHALL NOT attempt to construct or forge an unlisted one.
...
- **WHEN** the system selects page images for a leaflet
...
#### Scenario: Only signed variants are available
- **WHEN** the platform offers a fixed set of pre-signed image sizes
- **THEN** the system SHALL choose the smallest listed variant that remains legible for display and SHALL treat the absence of a smaller variant as a constraint to accept, not one to work around

### Requirement: Extract offers via vision with confidence gating
The system SHALL run each fetched page image through a vision extraction step to identify product name, price, and the location of the product's photograph on the page, and SHALL only emit an offer for an extracted item when the extraction meets a defined confidence bar for the offer's price. The product location SHALL be requested in the same extraction call, not a separate pass, and SHALL be gated independently of the offer itself.
